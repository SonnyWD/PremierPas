...
export class MakeTypeProfilNullable1750525998062 implements MigrationInterface {
  name = 'MakeTypeProfilNullable1750525998062';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user" ALTER COLUMN "type_profil" SET DEFAULT 'femme_enceinte'`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "user" ALTER COLUMN "type_profil" DROP DEFAULT`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."user_role_enum_old" AS ENUM('user', 'admin', 'premium')`,
    );
    await queryRunner.query(
      `ALTER TABLE "user" ALTER COLUMN "role" DROP DEFAULT`,
    );
