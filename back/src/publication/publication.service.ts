...
export class PublicationService {
  ...
  public async findPublication(publicationId: number, userId: number): Promise<Publication> {
  ...
  async createPublication(userId: number, createPublicationDto: CreatePublicationDto) {
  ...
  async updatePublication(publicationId: number, userId: number, updatePublicationDto: UpdatePublicationDto): Promise<Publication> {
    const existingPublication = await this.findPublication(publicationId, userId);
    Object.assign(existingPublication, updatePublicationDto);
    return await this.publicationRepository.save(existingPublication);
  }

  async removePublication(publicationId: number, userId: number): Promise<Publication> {
    const existingPublication = await this.findPublication(publicationId, userId);
  ...
  async findOnePublication(publicationId: number, userId: number,): Promise<Publication> {
    const existingPublication = await this.findPublication(publicationId, userId);
