...
export class AddGenderAndPayload1750604493755 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "baby" ADD "gender" character varying`,
    );
    await queryRunner.query(
      `UPDATE "tool" SET "path" = '' WHERE "path" IS NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tool" ALTER COLUMN "path" SET NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "tool" ALTER COLUMN "path" DROP NOT NULL`,
    );
