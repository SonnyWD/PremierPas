...
export class AppointementTypeService {
  ...
  async addDefaultAppointmentTypes() {
    ...
    for (const type of defaultTypes) {
      ...
      if (!existingType) {
        ...
        this.logger.log(
          `Type de rendez-vous "${type.title}" ajouté avec succès`,
        );
      } else {
  ...
  async create(createAppointmentTypeDto: CreateAppointmentTypeDto) {
    const newType = this.appointmentTypeRepository.create(
  ...
  async updateAppointement(
    id: number,
    updateAppointementTypeDto: UpdateAppointementTypeDto,
  ): Promise<AppointementType> {
    ...
    if (!appointementType) {
      throw new NotFoundException(
        `Ce type de rendez-vous médical avec l'ID ${id} est introuvable`,
      );
    }

    Object.assign(appointementType, updateAppointementTypeDto);

    const updatedAppointementType =
      await this.appointmentTypeRepository.save(appointementType);
  ...
  async removeAppointementType(id: number): Promise<AppointementType> {
    ...
    this.logger.log(
      `Type de rendez-vous "${appointementType.title}" supprimé avec succès`,
    );
