...
export class MedicalAppointementService {
  ...
  async createMedicalAppointement(
    createMedicalAppointementDto: CreateMedicalAppointementDto,
    userId: number,
  ) {
    const {
      date,
    ...
    try {
      ...
      if (pregnancyId && babyId) {
        throw new BadRequestException(
          'Un rdv médical ne peut pas être assigné à la fois à une grossesse et à un bébé',
        );
      }
      ...
      const appointmentType =
        await this.appointmentTypeRepository.findOne(appointementTypeId);
      if (!appointmentType) {
  ...
  async findOneAppointement(id: number): Promise<MedicalAppointement> {
    const medicalAppointement =
      await this.medicalAppointementRepository.findOne({
        where: { id },
      });
  ...
  async updateMedicalAppointement(
    id: number,
    updateMedicalAppointementDto: UpdateMedicalAppointementDto,
  ): Promise<MedicalAppointement> {
    const medicalAppointement =
      await this.medicalAppointementRepository.findOne({
        where: { id },
      });
    ...
    if (!medicalAppointement) {
      throw new NotFoundException(
        `Rendez-vous médical avec l'ID ${id} non trouvé`,
      );
    }
    ...
    const updatedAppointement =
      await this.medicalAppointementRepository.save(medicalAppointement);

    return updatedAppointement;
  }

  async removeMedicalAppointement(id: number): Promise<MedicalAppointement> {
    const appointement = await this.medicalAppointementRepository.findOne({
      where: { id },
    });
