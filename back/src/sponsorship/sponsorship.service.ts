...
export class SponsorshipService {
  ...
  async validateSponsorship(code: string, filleulId: number) {
    ...
    const godfather = await this.userRepository.findOne({ where: { referralCode: code } });
    if (!godfather) {
        throw new NotFoundException("Code parrain invalide.");
    }


    const filleul = await this.userRepository.findOne({ where: { id: filleulId } });
    if (!filleul) {
    ...
    return { message: "Parrainage validé ! 1 mois Premium offert à vous deux." };
}
...
async addPremium(user: User) {
    ...
    user.premiumUntil = new Date(currentDate.setDate(currentDate.getDate() + premiumDuration));
...
async updateSponsorship(id: number, updateSponsorshipDto: UpdateSponsorshipDto) {
