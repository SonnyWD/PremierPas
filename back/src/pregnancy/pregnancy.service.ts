...
export class PregnancyService {
  ...
  async create(userId: number, createPregnancyDto: CreatePregnancyDto) {
    ...
    const newPregnancy = this.pregnancyRepository.create({
      ...createPregnancyDto,
      user: { id: userId },
    });
  ...
  async updatePregnancy(
    userId: number,
    pregnancyId: number,
    updatePregnancyDto: UpdatePregnancyDto,
  ) {
    const pregnancy = await this.findPregnancy(userId, pregnancyId);

    const updatedPregnancy = this.pregnancyRepository.merge(
      pregnancy,
      updatePregnancyDto,
    );
  ...
  private async findPregnancy(userId: number, pregnancyId: number) {
    ...
    if (!pregnancy || pregnancy.user.id !== userId) {
      throw new NotFoundException(
        `Aucune grossesse trouvée avec l'ID ${pregnancyId} pour cet utilisateur.`,
      );
    }
